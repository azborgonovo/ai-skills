...
1. **State the user-visible symptom in plain terms**, as if explaining the bug report itself — not the tracker's internal jargon or issue ID. The agent should understand *what a user experiences* before it starts grepping.
...
3. **Enumerate exactly what you want back**: file paths, line numbers, actual code snippets (not paraphrases), config values, and an explicit stated hypothesis for why the current code produces the symptom. Ask for git history/blame on the relevant code if a recent change looks suspicious.
...
- **"Find the bug"** — no scope, no repo, no hypothesis. You'll get a generic tour of the codebase instead of a targeted answer.
