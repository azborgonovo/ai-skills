...
- Use the literal Unicode emoji character (🤖), not a `:robot:`-style shortcode, unless you've
  ...
  `references/trackers/jira.md`), so a shortcode posted there renders as the literal text
  ":robot:" instead of an emoji. When in doubt, the literal Unicode character is always safe.
