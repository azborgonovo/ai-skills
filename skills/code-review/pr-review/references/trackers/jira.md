...
Jira's markdown-to-ADF conversion does not expand emoji shortcodes (`:robot:`) the way GitLab/GitHub-flavored markdown do — posting `:robot:` in a comment elsewhere would render as the literal text ":robot:". This adapter is read-only (this skill never posts back to the work item), so it only matters if you quote work-item content verbatim in an inline comment — quote the text as-is rather than reformatting it.
