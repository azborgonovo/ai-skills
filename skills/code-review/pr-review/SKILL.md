...
Review of a merge or pull request that ends in inline draft comments on the diff. **You submit the review.**
...
The adapter file is the authority for that host's mechanics: URL parsing, auth check, the exact calls to fetch metadata and diffs, the local-clone path shape, and how to post comments (draft note vs. pending review) with that host's own gotchas. Read it now and follow it wherever a later step says "per the host adapter."
...
Using the metadata call from the host adapter, fetch the change's title, description, source/target branch, the SHAs needed for inline comment positions, its checks/pipeline status, and its web URL.
...
**What stays out of the comments**: the verdict, the summary paragraph, the "checked and clean" list, and the path to merge all belong to Step 8's output in the conversation. Skip a suggestion no author would act on — a style nit that automation should be catching — and where the same finding recurs across files, post the clearest occurrence once.

The host adapter names the bundled script that does the posting (`scripts/post_review_notes_gitlab.py` or `scripts/post_review_notes_github.py`, relative to this SKILL.md) and its exact invocation. Both read the same notes-file JSON shape, so drafting doesn't change with the host; both post as drafts (GitLab draft note, GitHub pending-review comment) that only the user can see until they submit; and both mark each comment with a trailing 🤖 plus one positionless note attributing the review as a whole ("Code reviewed using Sonnet 5 (high) 🤖"). Leave the marking to the script — writing it yourself only risks it landing twice.

Pass `--model "<your model name>"` so the attribution note names the reviewing model; you always know it from your environment context. Add `--effort "<level>"` only when you have a concrete, known effort setting for this session, never a guessed one.
...
<submit instructions from the host adapter — only when comments were posted>
```

**Marking posted findings** — tag each finding that went to the change with *(posted inline)* or *(posted as a general comment)*. It's the only way the reader can tell what the author will see from what stayed between the two of you.
...
- **Never** approve, reject, mark as reviewed, or submit the review — only post comments and notes. review-changes' verdict is information for the user, not an action to take on the host.
