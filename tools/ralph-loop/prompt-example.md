...
1. **Fetch subtasks** — use the `jira-cli` skill to list all subtasks of `PROJ-100`.
...
5. **Create a branch** — in that repository:
   ...
   2. Create and check out a new branch from `main` using the pattern: `{JIRA-TASK-NUMBER}-{short-slug}` (e.g. `PROJ-103-add-user-auth`)
